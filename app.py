...
# This file must be uploaded to your GitHub repository in the same folder as app.py
...
# ==========================================
# 3. WEB UI & FILE UPLOAD
# ==========================================
st.title("📅 Professional Roster Generator")

st.sidebar.header("1. Upload Previous Data")
uploaded_file = st.sidebar.file_uploader("Upload Last Month's Roster (Excel)", type=["xlsx"])

if not uploaded_file:
    st.info("👋 Please upload the previous month's Excel roster to extract duties and rotation history.")
    st.stop()
...
# Read Data from the Streamlit Uploaded File
...
# ==========================================
# 4. PRODUCTION SCHEDULING ENGINE
# ==========================================
if st.button("Generate Fair & Balanced Roster", type="primary"):
    with st.spinner("Executing Production Scheduling Engine & Formatting..."):
        ...
        try:
            ...
            st.error(f"Template file '{TEMPLATE_FILE}' not found. Please ensure it is uploaded to your GitHub repository.")
            st.stop()
